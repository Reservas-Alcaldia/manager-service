...
import { validateSolicitud } from "../Validations/solicitud.validation";
...
export class SolicitudService {
    ...
    async createSolicitud(dto: SolicitudDto): Promise<Solicitud> {
        ...
        const secretaria = await this.secretariaRepository.findOne({ where: { Nombre: dto.ministry } });
        if (!secretaria) {
        ...
        solicitud.Fecha_reserva = new Date(validatedDto.fecha_reserva);
